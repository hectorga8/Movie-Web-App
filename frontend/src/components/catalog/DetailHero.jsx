...
const DetailHero = ({ item, type, providers, onActionClick, pegi }) => {
  ...
  return (
    <section className="relative w-full bg-[#15181e] pt-24 pb-16 overflow-hidden flex justify-center">
      <div className="absolute inset-0 opacity-20 pointer-events-none">
        <img src={movieService.getBackdropUrl(item.backdrop_path)} className="w-full h-full object-cover grayscale" alt="" />
        <div className="absolute inset-0 bg-gradient-to-t from-[#15181e] via-[#15181e]/90 to-transparent"></div>
      </div>
      ...
      <div className="w-full max-w-[1200px] px-6 relative z-10">
        <div className="flex flex-col md:flex-row gap-12 lg:gap-16">
          ...
          <div className="w-[200px] md:w-[280px] shrink-0 mx-auto md:mx-0">
            <div className="rounded-[4px] overflow-hidden shadow-2xl border border-white/10 aspect-[2/3] bg-[#0d0e12]">
          ...
          <div className="flex-1 text-white text-center md:text-left">
            <nav className="label-uppercase text-[10px] mb-4 flex items-center justify-center md:justify-start gap-2 opacity-60 font-bold tracking-[2px]">
              ...
              <span>/</span>
              <span className="text-white uppercase">{type === 'movie' ? 'Película' : 'TV'}</span>
            </nav>
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-5 mb-8">
              ...
              <div className="flex items-center gap-4">
                <span className="text-md text-[#d5d7db] font-bold">{year}</span>
                <div className="px-2 py-0.5 border border-white/30 rounded-[3px] text-[10px] font-bold bg-white/5">{pegi}</div>
              </div>
              ...
              {providers && (
                ...
                >
                  ...
                  <span className="text-[10px] font-bold label-uppercase tracking-wider text-white">Ver en {providers.provider_name}</span>
                </a>
              )}
            </div>

            <div className="mb-10 max-w-2xl text-left">
              <h3 className="label-uppercase text-[10px] mb-3 opacity-40 font-bold tracking-[2px]">SINOPSIS</h3>
              <p className="body-relaxed text-[15px] md:text-[16px] text-[#d5d7db] leading-[1.6] italic opacity-90 line-clamp-4">
                {item.overview || "No hay sinopsis disponible."}
              </p>
              {creatorOrDirector && (
                <div className="mt-4 flex items-center justify-center md:justify-start gap-4">
                  <div className="w-1 h-6 bg-[#1060ff] rounded-full"></div>
                  <p className="text-[13px] font-bold text-white"><span className="opacity-50 font-normal">{type === 'movie' ? 'Director:' : 'Creador:'}</span> {creatorOrDirector}</p>
                </div>
              )}
            </div>
