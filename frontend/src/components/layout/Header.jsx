    ...
    <>
      <header className="sticky top-0 z-50 w-full border-b border-white/5 bg-[#0d0e12]/80 backdrop-blur-md h-16 md:h-20">
        <div className="w-full max-w-[1200px] mx-auto px-4 md:px-6 h-full flex items-center justify-between gap-4">
          ...
          {/* Lado Derecho: Acciones Desktop + Burger Mobile */}
      ...
      <div className={`fixed inset-0 z-40 lg:hidden transition-all duration-500 ease-in-out ${isMenuOpen ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0 pointer-events-none'}`}>
          ...
          <nav className="flex flex-col gap-6 mb-12">
            {navLinks.map((link) => (
              <Link 
                ...
                to={link.path} 
                onClick={() => setIsMenuOpen(false)}
                className="text-2xl font-bold text-white/90 hover:text-[#1060ff] transition-colors"
              >
                {link.name}
              </Link>
            ))}
          ...
          <div className="mt-auto pt-10 border-t border-white/10">
            {user ? (
              <div className="flex flex-col gap-6">
                ...
                <button 
                  onClick={handleLogout}
                  className="w-full py-4 rounded-xl bg-white/5 border border-white/10 text-white font-bold transition-all"
                >
                  Cerrar Sesión
                </button>
              </div>
              ...
              <div className="flex flex-col gap-4">
                ...
                <Link 
                  to="/registro" 
                  onClick={() => setIsMenuOpen(false)}
                  className="w-full py-4 rounded-xl bg-[#1060ff] text-white font-bold text-center shadow-lg shadow-[#1060ff]/20"
                >
                  Registrarse Gratis
                </Link>
              </div>
