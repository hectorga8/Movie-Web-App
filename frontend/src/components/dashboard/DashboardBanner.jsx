// ─────────────────────────────────────────────────────────────
// DashboardBanner.jsx - Versión CineBox
// Banner de bienvenida con progreso del reto anual de cine
// ─────────────────────────────────────────────────────────────

function DashboardBanner({ userName = "Usuario", moviesRead = 0, moviesGoal = 12, year = 2026 }) {
  const percentage = Math.min((moviesRead / moviesGoal) * 100, 100);
      ...
      <div className="relative z-10 flex-1">
        ...
        <p className="text-[#FDFCF7]/80 text-sm max-w-sm leading-relaxed">
          {moviesRead >= moviesGoal 
            ...
            : `Estás a ${moviesGoal - moviesRead} películas de completar tu objetivo anual. ¡Sigue así!`}
        </p>
      </div>
      ...
      <div className="relative z-10 w-full md:w-auto shrink-0 flex items-center justify-center">
        <div className="relative w-24 h-24 flex items-center justify-center">
          <svg className="absolute inset-0 w-full h-full -rotate-90">
            <circle cx="48" cy="48" r="44" stroke="currentColor" strokeWidth="8" fill="none" className="text-white/10"></circle>
