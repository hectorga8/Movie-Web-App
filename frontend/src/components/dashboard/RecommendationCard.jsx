...
function RecommendationCard({ image, title, author, description, rating = 5, badge = "", variant = "secondary" }) {
  ...
  return (
    <div className={`feature-card relative rounded-2xl shadow-sm transition-all overflow-hidden flex flex-col ${isMain ? "bg-[#283618] p-6 shadow-lg shadow-[#283618]/20" : "bg-white border border-[#606C38]/10 p-5"}`}>
      
      {/* Badge para la principal */}
      {isMain && (
        <div className="inline-flex items-center gap-1.5 bg-[#BC6C25] text-white text-[0.65rem] font-normal uppercase px-2.5 py-1 rounded-full mb-4 w-fit">
      ...
      {isMain && (
        <button className="mt-5 w-full px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-white text-sm font-normal border border-white/10 transition-all">
          Añadir a mi lista
        </button>
      )}
      ...
      {!isMain && (
        <button className="mt-4 w-fit text-[10px] font-normal uppercase text-[#283618] border-b border-[#283618]/20 hover:border-[#283618] transition-all">
          Guardar libro
        </button>
      )}
    </div>
