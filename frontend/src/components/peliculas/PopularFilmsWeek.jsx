    ...
    const fetchMovies = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          fetchedMovies = data.slice(0, 4);
        } else if (data && Array.isArray(data.results)) {
          fetchedMovies = data.results.slice(0, 4);
        }
        setMovies(fetchedMovies);
        
        // Fetch stats for these movies
        const statsObj = {};
        await Promise.all(fetchedMovies.map(async (movie) => {
          try {
            const mediaType = movie.media_type || (movie.title ? 'movie' : 'tv');
            const [wStats, reviews] = await Promise.all([
              watchlistService.getMediaStats(movie.id, mediaType),
              getReviewsForMedia(mediaType, movie.id).catch(() => [])
            ]);
            ...
            statsObj[movie.id] = { views: '0', lists: '0', likes: '0' };
          }
        }));
        setMovieStats(statsObj);
      } catch (error) {
  ...
  return (
    <div className="relative mb-10 group">
      ...
      <div className="grid grid-cols-4 gap-1.5">
        {movies.map((movie, i) => {
          const stats = movieStats[movie.id] || { views: '...', lists: '...', likes: '...' };
          return (
