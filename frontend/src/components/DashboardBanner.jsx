// ─────────────────────────────────────────────────────────────
// DashboardBanner.jsx
// Banner superior con el reto de lectura y progreso circular
// ─────────────────────────────────────────────────────────────
...
function DashboardBanner({ 
  ...
  // Cálculo del progreso circular (SVG)
  // El radio es 44, la circunferencia es ~276
  ...
  return (
    <div className="fade-up delay-2 relative w-full bg-[#283618] rounded-3xl p-6 md:p-8 mb-10 overflow-hidden shadow-lg shadow-[#283618]/20 flex flex-col md:flex-row md:items-center justify-between gap-6">
      ...
      {/* Blurs decorativos (idénticos al landing) */}
      <div className="pointer-events-none absolute -right-10 -top-10 w-48 h-48 bg-[#606C38] rounded-full blur-3xl opacity-40"></div>
      <div className="pointer-events-none absolute right-20 -bottom-10 w-32 h-32 bg-[#BC6C25] rounded-full blur-3xl opacity-20"></div>
      ...
      <div className="relative z-10">
        ...
        <h2 className="font-['Playfair_Display',Georgia,serif] text-3xl font-bold text-[#FDFCF7] mb-2">
          ¡Hola de nuevo, {userName}!
        </h2>
        
        <p className="text-[#FDFCF7]/80 text-sm max-w-sm leading-relaxed">
          {booksRead >= booksGoal 
            ? "¡Felicidades! Has alcanzado tu meta anual. 🎉" 
            : `Estás a ${booksGoal - booksRead} libros de alcanzar tu meta anual. ¡Sigue así!`}
        </p>
      </div>
      ...
      <div className="relative z-10 w-full md:w-auto shrink-0 flex flex-col items-center">
        <div className="relative w-24 h-24 flex items-center justify-center rounded-full border-4 border-[#FDFCF7]/10 mb-2">
          <svg className="absolute inset-0 w-full h-full -rotate-90">
            <circle 
              ...
              strokeWidth="4" 
              fill="none" 
              className="text-[#BC6C25] transition-all duration-1000 ease-out" 
        ...
        <p className="text-xs text-[#FDFCF7]/60 font-medium">libros completados</p>
      </div>
    </div>
  );
}
